...
public:
    vector<vector<int>> kClosest(vector<vector<int>>& points, int K) {
        multimap<int, vector<int>> closet;
        for (auto &point : points) {
            auto kc = point[0]*point[0] + point[1]*point[1];
            closet.insert(pair<int, vector<int>>(kc, point));
        }

        // for (const auto& c : closet) {
        //     cout << c.first << ' ' << c.second[0] << ' ' << c.second[1] << endl;
        // }

        vector<vector<int>> result;
        auto it = closet.begin();
        while (K--) {
            cout << it->first << endl;
